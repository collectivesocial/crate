...
- 2026-05-09 (work item #8 — crate cleanup & CI): Executed Mal's three-repo split for the `crate` side. Deleted `web/`, `landing/`, `deploy-web.yml`, `deploy-landing.yml`. Stripped web/landing scripts from root `package.json` and the `WEB_LEXICON` target from `codegen.sh`. Added `.github/workflows/test.yml` as a **single job** (not matrix) — `collective-social-api` uses a node-version matrix (20.x + 22.x) but crate pins Node 22 by policy, so a matrix adds no value. Mirrored `actions/checkout@v4` + `actions/setup-node@v4` from both siblings. Used `--if-present` on all lint/typecheck/test steps so CI stays green as scripts are added incrementally. `lexicons/` only gets `npm ci` — no test or codegen step in CI (codegen is a dev-time operation; hermetic builds rely on committed generated output). Sibling `open-social/test.yml` was the closer structural match (single job, same action versions); `collective-social-api/ci.yml` provided the node-cache and format-check pattern.
